...
public class SimpleClockWidget extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
                                int appWidgetId) {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.simple_clock_widget);
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(new Date());
        int hours = calendar.get(Calendar.HOUR_OF_DAY);
        int minutes = calendar.get(Calendar.MINUTE);
        views.setTextViewText(R.id.simple_time_text,
        ...
        views.setTextViewText(R.id.simple_date_text,
                String.format(Locale.ENGLISH, "%s, %s %s",
                        dayOfWeekList[dayOfWeek], date, monthList[month]));
    ...
    public void onEnabled(Context context) {
        ...
        WidgetUpdateManager.updateSimpleWidget(context);
    }
